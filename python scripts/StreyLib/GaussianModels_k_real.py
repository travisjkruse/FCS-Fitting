...
def k_real(z,a,r0,lambdaem,n):
    return np.where(RR(z,r0,lambdaem,n)>a**2,a**2/RR(z,r0,lambdaem,n),1.0)

#def gi(eta,xi,t,D,w0,a,R0,lambdaex,lambdaem,n):
#    return np.exp(-xi**2)*k_real(eta-np.sqrt(D*t)*xi,a,R0,lambdaem,n)*k(eta+np.sqrt(D*t)*xi,a,R0,lambdaem,n)/(8*D*t+w2(eta-np.sqrt(D*t)*xi,w0,lambdaex,n)+w2(eta+np.sqrt(D*t)*xi,w0,lambdaex,n))

def g_noexp(eta,xi,t,D,w0,a,R0,lambdaex,lambdaem,n):
    return m.sqrt(m.pi)*k_real(eta-np.sqrt(D*t)*xi,a,R0,lambdaem,n)*k_real(eta+np.sqrt(D*t)*xi,a,R0,lambdaem,n)/(8*D*t+w2(eta-np.sqrt(D*t)*xi,w0,lambdaex,n)+w2(eta+np.sqrt(D*t)*xi,w0,lambdaex,n))
...
# vol1 is integral over k_real(z) and the square normalizes the function g_hermite
# g(t)=g_hermite/vol1**2
def vol1(a,r0,lambdaem,n,C=None,D=None,lambdaex=None,w0=None,F=None,tf=None):
    return quad(k_real,0,maxz,args=(a,r0,lambdaem,n))[0]*m.pi
...
def gc_noexp(eta,xi,t,D,w11,w22,a1,a2,R1,R2,lambdaex1,lambdaem1,lambdaex2,lambdaem2,n,dz):
    return m.sqrt(m.pi)*k_real(eta-np.sqrt(D*t)*xi+dz,a1,R1,lambdaem1,n)*k_real(eta+np.sqrt(D*t)*xi,a2,R2,lambdaem2,n)/(8*D*t+w2(eta-np.sqrt(D*t)*xi+dz,w11,lambdaex1,n)+w2(eta+np.sqrt(D*t)*xi,w22,lambdaex2,n))
