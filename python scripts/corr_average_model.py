...
for i in range(len(experiments)):
    ...
    logfile=open(datadir+filename+'2.log',"w")
...
#            tf=1e-7,
        ...
        plt.ylabel('sigma')
        plt.xlabel('delta t in sec')
        plt.savefig(datadir+filename+color+'.png', bbox_inches='tight')
        plt.savefig(datadir+filename+color+'.pdf', bbox_inches='tight')
    logfile.close()
plt.show()

data3dGaussian.to_csv(datadir+'gaussian.csv')
data3dGaussianTriplet.to_csv(datadir+'gaussian_triplet.csv')
dataNumerical.to_csv(datadir+'Numerical.csv')
dataNumericalTriplet.to_csv(datadir+'NumericalTriplet.csv')
