...
from GaussianModels_k_real import modelFCS_t, modelFCS, modelFCS_n, modelFCS_nt, vol1, vol2, g_t, g_n, g, g_nt
...
experiments=pd.read_table(datadir+'All.txt')

for i in range(len(experiments)):
    filename=experiments['filename'][i]
    logfile=open(datadir+filename+'_realk.log',"w")
    ...
    for color in ['B','R']:
        ...
        resultS = modelFCS_t.fit(corrData[dataName],t=corrData['delta_t'],
            ...
            wxy=0.3,
            wz=1.0,
            F=Parameter(value=0.0,min=0.0,max=0.5,vary=True),
            tf=Parameter(5e-7,vary=True, min=1e-10, max=1e-4),
            D=Parameter(value=diffC, vary = False),
            weights=1./fitNoise)
...
#            tf=1e-7,
        ...
        plt.ylabel('sigma')
        plt.xlabel('delta t in sec')
        plt.savefig(datadir+filename+color+'_realk.png', bbox_inches='tight')
        plt.savefig(datadir+filename+color+'_realk.pdf', bbox_inches='tight')
    logfile.close()
plt.show()

data3dGaussian.to_csv(datadir+'gaussian_realk.csv')
data3dGaussianTriplet.to_csv(datadir+'gaussian_triplet_realk.csv')
dataNumerical.to_csv(datadir+'Numerical_realk.csv')
dataNumericalTriplet.to_csv(datadir+'NumericalTriplet_realk.csv')
